<template>
  <main>
    <header>
      <header-title title="Upload |Lakdan" />
    </header>
    <br />
    <article>
      <section>
        <div class="container form" style="color: #57646f">
          ...
          <form class="form" @submit.prevent="upload" enctype="multipart/form-data">
            ...
            <!--  -->
            <label for=""><b>HPS (Excel,PDF)</b></label>
            <div class="input-group mb-1 mt-1"><input @change="fileOne" type="file" class="form-control" id="fileInput" accept=".xlsx" /></div>
            <p id="message">{{ message.files.fileOne }}</p>
            <!--  -->
            <label for=""><b>BA HPS (PDF)</b></label>
            ...
            <!--  -->
            <label for=""><b>File excel HPE</b></label>
            ...
            <!--  -->
            <label for=""><b>ND EVPSPROC penyampaian RKS & HPE</b></label>
            ...
            <!--  -->
            <label for=""><b>ND persetujuan RKS & HPS</b></label>
...
import UploadModal from "../../components/lakdan/upload-modal.vue";
export default {
  ...
  methods: {
    ...
    fileOne(event) {
      ...
      if (event.target.files[0].type !== excel) {
        this.message.files.fileOne = "Disarankan file .xlsx/.pdf";
      }
    ...
    async upload() {
      ...
      if (this.form.files.fileSix === null) {
        let F = new File([""], "Tidak ada", { type: "text/plain", lastModified: moment().format("lll") });
        this.form.files.fileSix = F;
      }
      progress.start();
      ...
      setTimeout(async () => {
        ...
        formData.append("descOne", "HPS (Excel,PDF)");
        formData.append("descTwo", "BA HPS (PDF)");
        formData.append("descThree", "File excel HPE");
        formData.append("descFour", "ND EVPSPROC penyampaian RKS & HPE");
        formData.append("descFive", "ND persetujuan RKS & HPS");
        formData.append("descSix", "Dok. pendukung lainnya");
        progress.set(0.6);
        await axios
          .post("/api/udocument/rl/" + this.form.position, formData)
          .then((response) => {
            progress.set(0.8);
            this.message.isSuccess = "201 Created ✔";
            setTimeout(() => {
              this.$router.push("/dashboard/lakdan/mydocument");
              progress.done();
            }, 1000);
          })
          .catch((error) => {
